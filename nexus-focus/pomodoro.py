"""
NEXUS//FOCUS — Cyberpunk Pomodoro Timer
...
DEFAULT_COLORS = {
    "bg":     "#08001a",   # deep void
    "panel":  "#0e0625",   # panel bg
    "border": "#2a1450",   # idle border
    "lit":    "#9d4dff",   # active border
    "text":   "#e6e0ff",   # primary text
    "muted":  "#6a5090",   # dim text
    "work":   "#ff2e88",   # magenta — study
    "break":  "#00f0ff",   # cyan — break
    "accent": "#b829ff",   # purple — accent
    "ok":     "#00ffaa",   # green — status ok
}


# Ambient system-log lines (cycled while running)
AMBIENT_MSGS = [
    "PROC :: heap_compact()",
    "NET  :: heartbeat=OK",
    "MEM  :: 0x7FFE alloc",
...
class NexusFocus(tk.Tk):
    ...
    def __init__(self):
        ...
        # color binding registries
        self._wbinds = []   # (widget, opt, color_key)
        self._cibinds = []  # (canvas, item_id, opt, color_key)
    ...
    def _reg(self, widget, **role_map):
        """Register a widget's options against color keys."""
    ...
    def _reg_item(self, canvas, item_id, **role_map):
        """Register a canvas item's options against color keys."""
        for opt, key in role_map.items():
            try:
    ...
    def _build_header(self):
        ...
        for txt, role in [("NEXUS", "text"), ("//", "accent"), ("FOCUS", "work")]:
            l = tk.Label(tf, text=txt, font=("Consolas", 14, "bold"))
            self._reg(l, bg="bg", fg=role)
            l.pack(side="left")

        tag = tk.Label(tf, text="  v1.0 / control_room",
                       font=("Consolas", 8))
        ...
        self._reg(self.btn_settings,
                  bg="bg", fg="muted",
                  activebackground="panel", activeforeground="accent")
        self.btn_settings.pack(side="right", padx=(8, 0))

        self.lbl_uptime = tk.Label(
            rf, text="UPTIME 00:00:00", font=("Consolas", 9))
    ...
    def _build_footer(self):
        ...
        self.lbl_status = tk.Label(
            f, text="● STANDBY", font=("Consolas", 8))
    ...
    def _build_left(self, f):
        ...
        self.lbl_phase = tk.Label(f, text="STUDY",
        ...
        self.lbl_cycles = tk.Label(f, text="00",
        ...
        self.lbl_total = tk.Label(f, text="00:00:00",
        ...
        self.lbl_next = tk.Label(f, text="-> BREAK 05:00",
                                  font=("Consolas", 9))
    ...
    def _build_center(self, f):
        ...
        self.lbl_session = tk.Label(top, text="SESSION 01",
                                     font=("Consolas", 8))
        ...
        # Bottom: controls (pack first so they dock at bottom)
        ctrl = tk.Frame(f)
        self._reg(ctrl, bg="panel")
        ctrl.pack(side="bottom", pady=(0, 22))

        bs_kw = dict(
            font=("Consolas", 11), bd=0, relief="flat", cursor="hand2",
            width=4, height=2,
        )
        self.btn_reset = tk.Button(ctrl, text="↺",
                                    command=self._reset, **bs_kw)
        self._reg(self.btn_reset,
                  bg="bg", fg="text",
        ...
        self._reg(self.btn_play,
                  bg="bg", fg="work",
                  activebackground="border", activeforeground="work")
        self.btn_play.pack(side="left", padx=8)

        self.btn_skip = tk.Button(ctrl, text="⏭",
                                   command=self._skip, **bs_kw)
        self._reg(self.btn_skip,
                  bg="bg", fg="text",
                  activebackground="border", activeforeground="text")
        self.btn_skip.pack(side="left", padx=8)

        # Middle: ring (fills remaining space, canvas centered)
        rh = tk.Frame(f)
        self._reg(rh, bg="panel")
        rh.pack(side="top", fill="both", expand=True)

        size = 240
        self.ring_canvas = tk.Canvas(rh, width=size, height=size,
                                      highlightthickness=0)
        self._reg(self.ring_canvas, bg="panel")
        self.ring_canvas.pack(expand=True)  # centers in rh

        # Decorative outer ring
        outer = self.ring_canvas.create_oval(2, 2, size-2, size-2, width=1)
        self._reg_item(self.ring_canvas, outer, outline="border")

        # Inner background ring
        m = 16
        bg_arc = self.ring_canvas.create_oval(m, m, size-m, size-m, width=2)
        self._reg_item(self.ring_canvas, bg_arc, outline="border")

        # Progress arc
        self.ring_arc = self.ring_canvas.create_arc(
            m, m, size-m, size-m,
            start=90, extent=-360, width=3, style="arc",
        )
        self._reg_item(self.ring_canvas, self.ring_arc, outline="work")
        ...
        for i in range(12):
            ...
            r1, r2 = r - 26, r - 20
            x1 = r + r1 * math.cos(ang)
            y1 = r + r1 * math.sin(ang)
            x2 = r + r2 * math.cos(ang)
            y2 = r + r2 * math.sin(ang)
            ln = self.ring_canvas.create_line(x1, y1, x2, y2, width=1)
            self._reg_item(self.ring_canvas, ln, fill="border")

        # Timer text
        self.lbl_timer = self.ring_canvas.create_text(
            r, r - 6, text="25:00", font=("Consolas", 38, "bold"),
        )
        self._reg_item(self.ring_canvas, self.lbl_timer, fill="work")

        self.lbl_sub = self.ring_canvas.create_text(
            r, r + 30, text="STUDY MODE", font=("Consolas", 9),
        )
        self._reg_item(self.ring_canvas, self.lbl_sub, fill="muted")
    ...
    def _build_right(self, f):
        ...
        lf = tk.Frame(f)
        self._reg(lf, bg="bg")
        lf.pack(fill="both", expand=True, padx=14, pady=(8, 8))

        self.log_text = tk.Text(
            lf, font=("Consolas", 8),
            wrap="none", bd=0, padx=6, pady=6,
            highlightthickness=0,
        )
        self._reg(self.log_text,
                  bg="bg", fg="muted", insertbackground="muted")
        ...
        self.history_frame.pack(fill="x", padx=14, pady=(0, 12))
    ...
    def _history_row(self, parent, label, secs, is_today):
        ...
        l = tk.Label(row, text=label, font=("Consolas", 8),
                     width=7, anchor="w")
    ...
    # ════════════════════════════════════════════════════════
    def _render(self):
        total = WORK_SEC if self.phase == "work" else BREAK_SEC
        ...
        self.ring_canvas.itemconfig(self.lbl_timer,
                                     text=f"{m:02d}:{s:02d}", fill=col)
        self.ring_canvas.itemconfig(
            self.lbl_sub,
            text="STUDY MODE" if is_work else "BREAK MODE")
        self.ring_canvas.itemconfig(self.ring_arc,
                                     extent=-360 * ratio, outline=col)

        self.lbl_phase.config(
            text="STUDY" if is_work else "BREAK", fg=col)
        ...
        if is_work:
            self.lbl_next.config(text="-> BREAK 05:00",
                                  fg=self.colors["break"])
        else:
            self.lbl_next.config(text="-> STUDY 25:00",
                                  fg=self.colors["work"])

        self.lbl_session.config(text=f"SESSION {(self.cycles_today + 1):02d}")

        # left progress bar (shows elapsed)
    ...
    def _phase_end(self):
        if self.phase == "work":
            ...
            self._refresh_history()
            self._log(
                f"EVT  :: study_complete cycle={self.cycles_today}", "ok")
            self.phase = "break"
            self.remaining = BREAK_SEC
            self.lbl_status.config(text="● BREAK MODE",
                                    fg=self.colors["break"])
        else:
            self._log("EVT  :: break_complete", "break")
            self.phase = "work"
            self.remaining = WORK_SEC
            self.lbl_status.config(text="● STUDY MODE",
                                    fg=self.colors["work"])
        self._flash()
        self._render()
        self._tick_after = self.after(1000, self._tick)
    ...
    def _toggle(self):
        if self.running:
            ...
            phase_name = ("STUDY MODE" if self.phase == "work"
                          else "BREAK MODE")
    ...
    def _ambient_log_loop(self):
        if self.running:
            self._log(random.choice(AMBIENT_MSGS), "info")
        delay = random.randint(3500, 7500)
        self.after(delay, self._ambient_log_loop)

    def _autosave_loop(self):
    # 日付が変わったらtotal_secondsをリセット
        today = datetime.now().strftime("%Y-%m-%d")
    ...
    def _open_settings(self):
        ...
        tk.Label(
            win, text="◆ COLOR_CONFIG",
            font=("Consolas", 11, "bold"),
            bg=self.colors["bg"], fg=self.colors["work"],
        ).pack(pady=(20, 4))
        tk.Label(
            win, text="── modify visual parameters ──",
            font=("Consolas", 8),
        ...
        for key, lbl in items:
            ...
            vl = tk.Label(
                row, text=self.colors[key], font=("Consolas", 8),
                bg=self.colors["bg"], fg=self.colors["muted"],
                width=10, anchor="w",
            )
        ...
        tk.Button(
            bf, text="⟲ DEFAULTS", font=("Consolas", 9),
            bg=self.colors["bg"], fg=self.colors["muted"],
    ...
    def _apply_colors(self):
        ...
        for c, iid, opt, key in self._cibinds:
            try:
                ...
                pass
        self._refresh_log_tags()
        # re-apply state-dependent overrides
        if self.topmost:
